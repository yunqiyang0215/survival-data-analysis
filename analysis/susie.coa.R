...
geno.path <- sprintf("/gpfs/data/stephens-lab/uk-biobank-survival/yunqi/survpheno/ukb_asthma_self_report/data/geno_finemap_region202408/%s.rds", region)
...
num_cores = 10
t1 = proc.time()
fit <- ibss_from_ser(X, y, L = L, prior_variance = 1., prior_weights = rep(1/p, p), tol = 1e-3, maxit = niter,
                      estimate_intercept = TRUE, ser_function = fit_coxph, num_cores = num_cores)
t2 = proc.time()
res = list(fit, X, time = t2 - t1)
saveRDS(res, paste0("/gpfs/data/stephens-lab/uk-biobank-survival/yunqi/survpheno/ukb_asthma_self_report/result202408/coa/fit.susie.", region, ".rds"))
