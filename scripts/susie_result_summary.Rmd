...
for (i in 1:length(regions)){
  ...
  for (i in 1:length(cs$cs)){
    size = length(cs$cs[[i]])
    output = c(region, "AA", size, cs$purity[i, ])
  ...
  for (i in 1:length(cs$cs)){
    size = length(cs$cs[[i]])
    output = c(region, "COA", size, cs$purity[i, ])
    cs_summary = rbind(output, cs_summary) 
  }
  rm(res, fit, X)
  ...
  for (i in 1:length(cs$cs)){
    size = length(cs$cs[[i]])
    output = c(region, "AOA", size, cs$purity[i, ])
    cs_summary = rbind(output, cs_summary) 
  }
  rm(res, fit, X)
}
...
for (i in 1:5){
  ...
  plot(-log10(gwas[, "p.value.spa"]), col = colors, xlab = "SNP", ylab = "-log10(p-value)", cex = 0.8, pch = 20, main = paste0(region, ": AA"))
  rm(gwas, res, X, fit)
  ...
  plot(-log10(gwas[, "p.value.spa"]), col = colors, xlab = "SNP", ylab = "-log10(p-value)", cex = 0.8, pch = 20, main = paste0(region, ": COA"))
  rm(gwas, res, X, fit)
  ...
  plot(-log10(gwas[, "p.value.spa"]), col = colors, xlab = "SNP", ylab = "-log10(p-value)", cex = 0.8, pch = 20, main = paste0(region, ": AOA"))
  rm(gwas, res, X, fit)
}
```
