...
for (i in 1:length(file.survival)){
  ...
  plot(dat$POS, -log10(dat$P), pch = 20, cex = 0.5, main = region_name, ylab = "log10-pval", xlab = "Position", ylim = c(0, max(-log10(dat$P), -log10(dat$p.value.spa)) + 1) )
  points(dat$POS, -log10(dat$p.value.spa), pch = 20, cex = 0.5, col = "red")
  legend("topright", legend = c("logistic.gwas", "survival.gwas"), col = c("black", "red"), pch = 20)
}
